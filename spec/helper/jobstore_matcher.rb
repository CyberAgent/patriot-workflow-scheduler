...
module JobStoreMatcher
  ...
  RSpec::Matchers.define :be_waited_in do |job_store|
    ...
    failure_message do |job|
      "expected #{Patriot::JobStore::JobState::WAIT} but #{job_store.get_job(job.job_id)[:state]}"
    end
  end
  ...
  RSpec::Matchers.define :be_succeeded_in do |job_store|
    ...
    failure_message do |job|
      "expected #{Patriot::JobStore::JobState::SUCCEEDED} but #{job_store.get_job(job.job_id)[:state]}"
    end
  end
  ...
  RSpec::Matchers.define :be_failed_in do |job_store|
    ...
    failure_message do |job|
      "expected #{Patriot::JobStore::JobState::FAILED} but #{job_store.get_job(job.job_id)[:state]}"
    end
  end
  ...
  RSpec::Matchers.define :be_running_in do |job_store|
    ...
    failure_message do |job|
      "expected #{Patriot::JobStore::JobState::RUNNING} but #{job_store.get_job(job.job_id)[:state]}"
    end
  end
end
